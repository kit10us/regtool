@set "REGTOOL=C:\ws\SCBU\tools\RegTool\RegTool\bin\Debug\RegTool.exe"
...
@for /F "tokens=1,2*" %%i in ('%REGTOOL% query "%1\SOFTWARE\Microsoft\VisualStudio\SxS\VS7" /v "11.0"') DO (
	@if "%%i"=="11.0" (
...
@for /F "tokens=1,2*" %%i in ('%REGTOOL% query "%1\SOFTWARE\Wow6432Node\Microsoft\VisualStudio\SxS\VS7" /v "11.0"') DO (
	@if "%%i"=="11.0" (
		@SET "VS110COMNTOOLS=%%k"
	)
)
